...
def main():
    ...
    st.subheader("Iris Species Predictor - Logistic Regression")

    col5, col6, col7, col8 = st.columns(4)

    with col5:
        st.subheader("Select Petal Length")
        pl = st.number_input("Enter the desired petal length", min_value=0.0, step=0.1, value = 0.0)
    with col6:
        st.subheader("Select Petal Width")
        pw = st.number_input("Enter the desired petal width", min_value=0.0, step=0.1, value = 0.0)
    with col7:
        st.subheader("Select Sepal Length")
        sl = st.number_input("Enter the desired sepal length", min_value=0.0, step=0.1, value = 0.0)
    with col8:
        st.subheader("Select Sepal Width")
        sw = st.number_input("Enter the desired sepal width", min_value=0.0, step=0.1, value = 0.0)

    if st.button("Predict Species"):
        species = predict_species(df, pw, pl, sw, sl)
        st.write(f"The predicted species for the input values is: **{species[0]}**")
